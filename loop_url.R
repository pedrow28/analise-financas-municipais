...
receita_morada_tratada <- df_receita %>% mutate(pos_neg = case_when(str_detect(Valor, "-") ~ "Negativo",
                                          TRUE ~ "Postivo")) %>% ##Indicando polaridade
  mutate(Valor = str_remove_all(Valor, "-")) %>%
  separate(Valor, into = c("Reais", "Centavos"), sep = ",", remove = FALSE) %>%
  filter(!is.na(Centavos)) %>% 
  mutate(Reais = as.numeric(Reais) * 1000,
         Centavos = as.numeric(Centavos) / 100) %>% 
  mutate(valor_soma = Reais + Centavos) %>% 
  mutate(Valor = case_when(pos_neg == "Negativo" ~ valor_soma * -1,
                           TRUE ~ valor_soma)) %>% 
  select(-c(Reais, Centavos, valor_soma, pos_neg))
  

saveRDS(receita_morada_tratada, "receitas/receita_consolidada_morada.RDS")
